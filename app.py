...
from event_logger import log_event  # Custom logger that writes to MongoDB GridFS
from ultralytics import YOLO

# MongoDB Setup (replace with your secure URI)
...
user_name = st.session_state.logged_user
...
if register:
    if not user_name:
    ...
    elif collection.find_one({"name": user_name}):
        ...
        if all(v is not None for v in captured_encodings.values()):
            ...
            st.success(f"✅ All 5 poses saved for {user_name}")

# ========== RECOGNITION ==========
if recognize:
    documents = list(collection.find({"name": user_name}))
    if not documents:
        st.warning("⚠️ No registered encodings found for current user.")
    else:
        known_encodings = []
        known_names = []
        ...
        while recognize:
            ...
            for face in detected_faces:
                ...
                if employee_name != user_name:
                    if not last_logged_events["unknown_face"] or (now - last_logged_events["unknown_face"]).total_seconds() > COOLDOWN_SECONDS:
                        log_event(user_name, "unknown_face", frame)
                        last_logged_events["unknown_face"] = now

                if device_detected:
                    if not last_logged_events["smart_device"] or (now - last_logged_events["smart_device"]).total_seconds() > COOLDOWN_SECONDS:
                        log_event(user_name, "smart_device", frame)
                        last_logged_events["smart_device"] = now
